...
static int bq27x00_battery_resume(struct device *dev)
{
        ...
        struct bq27x00_device_info *di = i2c_get_clientdata(client);
        pr_warn("%s\n", __func__);
        pr_debug("%s:before schedule_delayed_work(&di->work, msecs_to_jiffies(100))\n",__func__);
        schedule_delayed_work(&di->work, msecs_to_jiffies(100));
        return 0;
}
#endif
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		if (*blank == FB_BLANK_UNBLANK)
			bq27x00_battery_resume(&client->dev);
		else if (*blank == FB_BLANK_POWERDOWN)
			bq27x00_battery_suspend(&client->dev);
	}
	return 0;
}
#endif
