...
static char androidbootmode[9];//liyong2 2015.4.15 read androidboot.mode
static int __init android_boot_mode_setup(char *bootmode)
{
	char *str;
	int ints[9];
	str = get_options(bootmode, ARRAY_SIZE(ints), ints);
	strlcpy(androidbootmode,str,8);
	return 1;
}
early_param("androidboot.mode", android_boot_mode_setup);
...
static void mdp3_ctrl_pan_display(struct msm_fb_data_type *mfd)
{
	...
	if (!splash_done || mdp3_session->esd_recovery == true) {
		if(panel && panel->set_backlight)
		{
			if(strncmp(androidbootmode, "charger", ARRAY_SIZE("charger"))) //liyong2 2015.4.15 if bootmode=charger, do not turn on backlight here; else if boot mode is ffbm or normal, turn on backlight
			panel->set_backlight(panel, panel->panel_info.bl_max);
		}
		splash_done = true;
		mdp3_session->esd_recovery = false;
