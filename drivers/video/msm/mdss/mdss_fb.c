	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pdata = dev_get_platdata(&mfd->pdev->dev);
	if(!pdata){
		pr_info("not available pdata\n");
		return count;
	}
	if(mdss_dsi_is_panel_off(pdata)){
		pr_info("Dsi is not power on\n");
		return count;
	}
	ctrl = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	if(!ctrl) {
		pr_info("not available ctrl\n");
		return count;
	}
	/*
...
static DEVICE_ATTR(msm_fb_dispparam, 0644, NULL, mdss_fb_set_dispparam);//add by liyong2 2015.4.20 for lcd temprature
...
static struct attribute *mdss_fb_attrs[] = {
	...
	&dev_attr_msm_fb_dispparam.attr,//add by liyong2 2015.4.20 for lcd temprature
	NULL,
};
