...
//baron add start 0
void mdss_dsi_panel_gpio_request(struct mdss_dsi_ctrl_pdata *ctrl);
void mdss_dsi_panel_gpio_free(struct mdss_dsi_ctrl_pdata *ctrl);
//baron add end 0
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		#ifdef CONFIG_LXF_P5000_COMMON //liyong2 2015.3.18
		if (gpio_is_valid(ctrl_pdata->bklt_en_gpio))
			gpio_set_value((ctrl_pdata->bklt_en_gpio), 1);
		#endif
							...
							u32 bl_level)
{
	...
	#ifdef CONFIG_LXF_P5000_COMMON
	static u32 old_bl_lvl=0;
	#endif
	...
	//not found LCD in lk,set bl_level to 0,add by zhouhehe.	
	 ...
	 */
	 if(bl_level>10)  //add by liyong2 for over temperature 2015.5.11
		bl_level = (int)(bl_level*215/255);//215=320lux   235=360lux 255=390lux

	if ((bl_level < pdata->panel_info.bl_min) && (bl_level != 0))
		bl_level = pdata->panel_info.bl_min;
	#ifdef CONFIG_LXF_P5000_COMMON
	if((old_bl_lvl == 0 && bl_level != 0) || bl_level==0){
		pr_err("%s bl_level=%d\n",__func__,bl_level);
	}
	#endif
...
#ifdef CONFIG_LXF_P5000_COMMON  //liyong2 modified by liyong2 2015.3.18
int  mdss_dsi_panel_lcden_gpio_ctrl(struct mdss_dsi_ctrl_pdata *ctrl_pdata, int on)
{
	...
	if(on){
		...
		if (gpio_is_valid(ctrl_pdata->lcden_gpio)){
			...
			msleep(5);//wait for analog voltage stable
		}
	}else{
		if (gpio_is_valid(ctrl_pdata->lcden_gpio)){
...
//baron add end 1
	...
	//add by liyong2 2015.4.20 for lcd temprature
