...
#define IST510E_IF_DEVICE_NAME  "ist510e_debug"
/*------------------------------------------------------------------------------
 *
 *-----------------------------------------------------------------------------*/
 ...
 *-----------------------------------------------------------------------------*/
static size_t
hideep_download_uc(struct hideep_data *ts, const char __user *uc, int offset, size_t count)
{
		...
		dbg_log("start offset = 0x%0x, count = %d",offset, count);
		
		len += count;
		len -= 47*1024;
		if( len > 2048)
			goto istcore_download_uc_err;
		dbg_log("processing");
		...
		hideep_fuse_ucode(private_ts->client, ucode, count, offset);
#else
		...
		dbg_log("ist510e_download_uc(%d)", count);
		return count;
	istcore_download_uc_err:
		dbg_err("data(size = %d) to be downloaded is oversize ",(offset)+ count);
		return -1;
}
/*******************************************************************************
 * file operations
...
hideep_iface_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
    ...
    //---------------------------------
    if( ((*offset)&ADDR_UC) == ADDR_UC )
    {
        dbg_log("download fw vr, offset = 0x%x, count = %d", (unsigned int)(*offset),count);
        wr_len = hideep_download_uc(drv_info->ts, buf, (int)((*offset)&0xffff), count);
    }
    else
    {
