...
#include <linux/input/hideep_blueberry_ts.h>
#include "hideep.h"
...
//		ts->power = ts->pdata->power;
    ...
    if(check_version(ts->client,vs)>=0){
        if((vs[8] & 0xf0) ==0)
            sprintf(tp_ver, "[Hideep] O-film FW:v%d.%02d\n", 
                         vs[__HIDEEP_MAJOR_VERSION__+6], vs[__HIDEEP_MINOR_VERSION__+6]);
        else
            sprintf(tp_ver, "[Hideep] Gis FW:v%d.%02d\n", 
                         vs[__HIDEEP_MAJOR_VERSION__+6], vs[__HIDEEP_MINOR_VERSION__+6]);
    }else{
        sprintf(tp_ver, "can't read out version no.");
    }
	init_tp_fm_info(0,tp_ver,"IST940E");
#endif
#if defined(CONFIG_FB)
    ...
    queue_delayed_work(ts->hideep_fw_wq, &ts->fw_work,1000);
    procfs_create();
