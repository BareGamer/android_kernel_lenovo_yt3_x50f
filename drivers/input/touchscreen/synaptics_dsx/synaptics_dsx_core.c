		...
		unsigned int intr_count)
{
	...
	printk("%s: F12 ctrl_8_offset = %d\n", __func__, ctrl_8_offset);
	if (query_5.ctrl8_is_present) {
		...
		printk("%s: rmi4_data->sensor_max_x = %d\n", __func__, rmi4_data->sensor_max_x);
		printk("%s: rmi4_data->sensor_max_y = %d\n", __func__, rmi4_data->sensor_max_y);
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F12:
				...
				printk("%s: F12 data base = 0x%04x\n", __func__, fhandler->full_addr.data_base);
				printk("%s: F12 control base = 0x%04x\n", __func__, fhandler->full_addr.ctrl_base);
				printk("%s: F12 command base = 0x%04x\n", __func__, fhandler->full_addr.cmd_base);
				printk("%s: F12 query base = 0x%04x\n", __func__, fhandler->full_addr.query_base);
