...
#if 0
#define hideep_swd_clk_pin hideep_gpio_i2c_scl_pin
#define hideep_swd_dat_pin hideep_intb_pin

#define hideep_swd_set_output() \
		do{\
			gpio_direction_output(hideep_swd_dat_pin,1);\
		}while(0);

#define hideep_swd_set_input() \
		do{\
			gpio_direction_input(hideep_swd_dat_pin);\
		}while(0);
#define HIDEEP_SWD_CLK_SET(a) gpio_direction_output(hideep_swd_clk_pin, a)
#define HIDEEP_SWD_DAT_SET(a) gpio_direction_output(hideep_swd_dat_pin, a)
#define HIDEEP_SWD_DAT_GET() (__gpio_get_value(hideep_swd_dat_pin)?1:0)
#endif
...
int hideep_blaster_init(void)
{
	...
	if (g_cmpid.u8compclass != 0) {
		...
		for (i = 0; i < 10/*0x3C0*/; i++) {
			...
			if(ret != 0)
				goto error;
			dbg_log("sub component [%d] Addr (0x%08x)", i, g_u32compbase[i]);
