...
#define TEST_MODE_RESPONS_ADDR	0x000C
#define TEST_RESULT_ADDR		0x1000

#define COMMAND_4RC				0x00
#define COMMAND_1RC				0x01
#define COMMAND_EOP				0x02
#define COMMAND_4RCF			0x03
#define COMMAND_SLFTST			0x04

#define TX_NUM		28
#define RX_NUM		42
#define SIZE_NODE	2
#define DATA_LEN				(TX_NUM * RX_NUM * SIZE_NODE)
...
void hideep_test_mode(struct hideep_data *ts)
{
	...
	framebuf = kmalloc(DATA_LEN, GFP_KERNEL);
	if(framebuf ==NULL)
	{
		dbg_err("can't not memmory alloc\n");
	...
	tempframe = kmalloc(DATA_LEN, GFP_KERNEL);
	if(tempframe ==NULL)
	{
		dbg_err("can't not memmory alloc\n");
	...
	// check response
	mdelay(500);
	ret = hideep_i2c_read(ts->client, TEST_MODE_RESPONS_ADDR, 1, buf);
	//if(ret == ?)
	{
		// todo
	}
	if(buf[0] != 1)
	{
		dbg_err("Reading 4RCF is failed\n");
		goto exit_tempframe_alloc_hideep_test_mode;
	}

	// save the data
	ret = hideep_i2c_read(ts->client, TEST_RESULT_ADDR, DATA_LEN, (unsigned char *)framebuf);
	...
	if(buf[0] != 1)
	{
		dbg_err("Reading 4RC is failed\n");
		goto exit_tempframe_alloc_hideep_test_mode;
	}

	// save the data
	ret = hideep_i2c_read(ts->client, TEST_RESULT_ADDR, DATA_LEN, (unsigned char *)tempframe);
	...
	for (r=0; r<RX_NUM; r++)
	{
		if (ts->RXshortResult[r] == 0)
		{
			...
			{
				ts->RXopenResult[r] = 1;
				dbg_log("RX Channel Open = %d\n", r);
			}
			else
			{
				ts->RXopenResult[r] = 0;
				dbg_log("RX 4RC Pass = %d\n", r);
			}
		}
		else
		{
			ts->RXopenResult[r] = 0;
			dbg_log("Skip to check 4RC RX = %d\n", r);
		}
	}
	...
	for(t=0; t<TX_NUM; t++)
	{
		if (ts->TXshortResult[t] == 0)
		{
			...
			{
				ts->TXopenResult[t] = 1;
				dbg_log("TX Channel Open = %d\n", t);
			}
			else
			{
				ts->TXopenResult[t] = 0;
				dbg_log("TX CMP Pass = %d\n", t);
			}
		}
		else
		{
			ts->TXopenResult[t] = 0;
			dbg_log("Skip to check CMP TX = %d\n", t);
		}
	}
