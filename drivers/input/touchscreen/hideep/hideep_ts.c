...
#define HIDEEP_VTG_MIN_UV		2600000
...
static void hideep_swd_uninit(void)
{
    ...
    #if 0
    if (private_ts->ts_pinctrl) {
        for(retry_time = 0; retry_time < HIDEEP_CHANGE_I2C_GPIO_RETRY_TIME;retry_time++){
		    ret = pinctrl_select_state(private_ts->ts_pinctrl,
					private_ts->pinctrl_state_iic);
            dbg_fun("retry_time = %d",retry_time);
            if(ret >= 0)break;
        }
        if(retry_time >=HIDEEP_CHANGE_I2C_GPIO_RETRY_TIME){
			pr_err("failed to select pin to iic,ret = %d\n",ret);
			goto pinctrl_deinit;
		}
    }
    #endif
    ...
    pinctrl_deinit:
	if (private_ts->ts_pinctrl) {
		if (IS_ERR_OR_NULL(private_ts->pinctrl_state_release)) {
			devm_pinctrl_put(private_ts->ts_pinctrl);
			private_ts->ts_pinctrl = NULL;
		} else {
			ret = pinctrl_select_state(private_ts->ts_pinctrl,
					private_ts->pinctrl_state_release);
			if (ret)
...
//extern struct pinctrl_state *tmp;
static int hideep_swd_init(void)
{
   ...
   // int retry_time;
    ...
    if (private_ts->ts_pinctrl) {
        ...
        {
		    ret = pinctrl_select_state(private_ts->ts_pinctrl,
					tmp/*private_ts->pinctrl_state_gpio*/);
            dbg_fun("retry_time = %d",retry_time);
		    if (ret >= 0) {
                break;
		    }
        ...
        if(retry_time >=HIDEEP_CHANGE_I2C_GPIO_RETRY_TIME){
		    	dev_err(&client->dev,
				"failed to select pin to gpio,ret = %d\n",ret);
			    goto pinctrl_deinit;
        }
	}
    #endif
    /* scl, sda gpio info */
	private_ts->pdata->gpio_scl= of_get_named_gpio_flags(np,
			"hideep,scl-gpio", 0, NULL);
	private_ts->pdata->gpio_sda= of_get_named_gpio_flags(np,
			"hideep,sda-gpio", 0, NULL);
...
#else
#define HIDEEP_SCL_PIN 18
#define HIDEEP_SDA_PIN 19
#define reg_i2c2_scl_setting	(IOMEM(0xFD510000)+ 0x1000 + (0x10 * HIDEEP_SCL_PIN))
#define reg_i2c2_sda_setting	(IOMEM(0xFD510000)+ 0x1000 + (0x10 * HIDEEP_SDA_PIN))
static void hideep_swd_uninit(void)
{
	unsigned int tmp;
	dbg_fun();
	...
	tmp = readl_relaxed(reg_i2c2_scl_setting);
	dbg_log("scl setting = 0x%08x", tmp);
	tmp&= ~(0x0f<<2);
	tmp |= 2<<2;
    writel_relaxed(tmp,reg_i2c2_scl_setting);
	tmp = readl_relaxed(reg_i2c2_scl_setting);
	dbg_log("scl setting = 0x%08x", tmp);

	tmp = readl_relaxed(reg_i2c2_sda_setting);
	dbg_log("sda setting = 0x%08x", tmp);
	tmp&= ~(0x0f<<2);
	tmp |= 2<<2;
    writel_relaxed(tmp,reg_i2c2_sda_setting);
	tmp = readl_relaxed(reg_i2c2_sda_setting);
	dbg_log("sda setting = 0x%08x", tmp);
...
static int vr_update(struct hideep_data *ts)
{
    ...
    if (ret < 0)
        pr_err("[TSP] check_vr fail! [%d]", ret);
    else
    {
...
static void hideep_get_data(struct hideep_data *ts)
{
    ...
    for (i = 0; i < I2C_RETRY_CNT; i++)
    {
        ...
        {
#if DEBUG_PRINT
            pr_info("[TSP] : TOUCH_READ_REGS_LEN [%d] \n", ret);
#endif
            break; // i2c success
        }
    }
...
//screen
    if (touch_count > 0 && touch_count < TOUCH_MAX_COUNT+1)
    {
        for (i = 0; i < I2C_RETRY_CNT; i++)
        {
            ...
            {
#if DEBUG_PRINT
                pr_info("[TSP] hideep_get_data : TS_READ_START_ADDR [%d] \n", ret);
#endif
                break; // i2c success
            }
        }
        ...
        else
        {
            for (i = 0; i < touch_count*TOUCH_READ_SIZE; i = i + TOUCH_READ_SIZE)
            {
                ...
                dbg_log("x=%d,y=%d,z=%d,w=%d,flag=%d,type=%d,index=%d",tFinger.x,tFinger.y,
                        tFinger.z,tFinger.w,tFinger.flag,tFinger.type,tFinger.index);
				index = tFinger.index;
				type = tFinger.type;
				flag = tFinger.flag;
...
//key
	if (key_count > 0)
	{
		for (i = 0; i < I2C_RETRY_CNT; i++)
        {
            ...
            {
#if DEBUG_PRINT
                pr_info("[TSP] hideep_get_data : TS_KEY_READ_START_ADDR [%d] \n", ret);
#endif
   	            break; // i2c success
       	    }
       	}
...
static int hideep_ts_parse_dt(struct device *dev, struct hideep_platform_data *hideep_pdata){
	...
	hideep_pdata->power_ldo_gpio = of_get_named_gpio_flags(np, "hideep,power_ldo-gpio",
				0, NULL);
	if (!gpio_is_valid(hideep_pdata->power_ldo_gpio))
		dev_err(dev, "power ldo gpio not specified \n");
	/* reset, irq gpio info */
	hideep_pdata->gpio_reset= of_get_named_gpio_flags(np,
			"hideep,reset-gpio", 0, NULL);
	if (!gpio_is_valid(hideep_pdata->gpio_reset))
		dev_err(dev, "reset gpio not specified\n");
	hideep_pdata->gpio_int= of_get_named_gpio_flags(np,
			"hideep,irq-gpio", 0, NULL);
	if (!gpio_is_valid(hideep_pdata->gpio_int))
		dev_err(dev, "interrupt gpio not specified\n");

#ifdef	HIDEEP_TP_VENDOR
	hideep_pdata->gpio_tp_vendor = of_get_named_gpio_flags(np,
			"hideep,tp-vendor-gpio", 0, NULL);
	if (!gpio_is_valid(hideep_pdata->gpio_tp_vendor))
		dev_err(dev, "tp vendor gpio not specified\n");
#endif
...
static int hideep_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if(private_ts->pdata->int_enable)
...
static int hideep_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ts == NULL)
    {
        pr_info("[TSP] %s: failed to create a state of hideep-ts\n", __FUNCTION__);
        ret = -ENOMEM;
...
//		ts->power = ts->pdata->power;
    ...
    if (!ret && ts->ts_pinctrl) {
       ...
       if(gpio_is_valid(ts->pdata->gpio_int)){
           ...
           msleep(20);
	       ret = gpio_get_value(ts->pdata->gpio_int);
	       dev_info(&client->dev,
				"INT status is %d \n", ret);
	       ts->irq = ts->client->irq = gpio_to_irq (ts->pdata->gpio_int);
	}
      #ifdef	HIDEEP_TP_VENDOR
    ...
    if(check_version(ts->client,vs)>=0){
        if((vs[8] & 0xf0) ==0){
            sprintf(tp_ver, "[FW]v%d.%02d,[IC]IST700", 
                         vs[__HIDEEP_MAJOR_VERSION__+6], vs[__HIDEEP_MINOR_VERSION__+6]);
            init_tp_fm_info(0,tp_ver,"O-film");
        }else{
            sprintf(tp_ver, "[FW]v%d.%02d,[IC]IST700", 
                         vs[__HIDEEP_MAJOR_VERSION__+6], vs[__HIDEEP_MINOR_VERSION__+6]);
            init_tp_fm_info(0,tp_ver,"Mutto");
        }
    }else{
    ...
    if (ts->client->irq){
        ret = request_threaded_irq(ts->client->irq, NULL, hideep_irq_handler, /*IRQF_ONESHOT | IRQ_TYPE_EDGE_FALLING */(IRQF_TRIGGER_LOW | IRQF_ONESHOT), ts->client->name, ts);
        printk("request_threaded_irq, ret = 0x%x", ret);
        if (ret > 0){
            printk("Can't allocate irq %d, ret %d", ts->client->irq, ret);
            ret = -EBUSY;
    ...
    printk("[TSP] %s: Start touchscreen. name: %s, irq: %d\n", __FUNCTION__, ts->client->name, ts->client->irq);
...
err_power_on_failed:
    pr_info("[TSP] %s: failed to power on\n", __func__);
...
err_power_init_failed:
    pr_info("[TSP] %s: failed to power init\n", __func__);
...
err_ts_parse_dt_failed:
    pr_info("[TSP] %s: failed to ts parse dt\n", __func__);
err_mem_alloc_failed:
    pr_info("[TSP] %s: failed to allocate memory\n", __func__);
