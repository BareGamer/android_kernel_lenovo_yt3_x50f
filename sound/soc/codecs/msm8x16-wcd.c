...
static int msm8x16_spk_hs_switch_ctrl(struct snd_soc_card *data, bool value)
{
	...
	{
              printk("%s, set spk_hs_switch_gpio = %d \n", __func__, value);
		gpio_set_value_cansleep(pdata->spk_hs_switch_gpio, value);
              msleep(1);
	}
	else
	{
...
static int msm8x16_wcd_hs_spk_switch_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
       struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	if (gpio_is_valid(pdata->spk_hs_switch_gpio))
	{
		ret = gpio_get_value_cansleep(pdata->spk_hs_switch_gpio);
	}
	else
	{
		ret = -EINVAL;
...
static int msm8x16_wcd_hs_spk_switch_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int ret;
       ...
       switch (ucontrol->value.integer.value[0]) {
	case 0:
		ret = msm8x16_spk_hs_switch_ctrl(codec->card, false);
		break;
	case 1:
		ret = msm8x16_spk_hs_switch_ctrl(codec->card, true);
		break;
	default:
		ret = -EINVAL;
	}
...
#if defined(CONFIG_SPEAKER_HEADPHONE_SWITCH)
static const char * const msm8x16_wcd_hs_spk_switch_text[] = {
        "HS", "SPK"};
static const struct soc_enum msm8x16_wcd_hs_spk_switch_enum[] = {
        SOC_ENUM_SINGLE_EXT(2, msm8x16_wcd_hs_spk_switch_text),
};
#endif
...
#if defined(CONFIG_SPEAKER_HEADPHONE_SWITCH)
       SOC_ENUM_EXT("HS SPK Switch", msm8x16_wcd_hs_spk_switch_enum[0],
		msm8x16_wcd_hs_spk_switch_get, msm8x16_wcd_hs_spk_switch_put),
#endif
