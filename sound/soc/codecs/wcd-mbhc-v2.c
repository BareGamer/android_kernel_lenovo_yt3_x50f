...
#ifdef CONFIG_HEADSET_EUO_US_SWITCH
...
#if !defined(CONFIG_SPEAKER_EXT_PA) //p5000 ext pa,don't on hph pa here
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		wcd_mbhc_jack_report(mbhc, &mbhc->headset_jack,
				mbhc->hph_status, WCD_MBHC_JACK_MASK);
        #if !defined(CONFIG_SPEAKER_EXT_PA) //p5000 ext pa,don't off hph pa here
...
static bool wcd_is_special_headset(struct wcd_mbhc *mbhc)
{
	...
	result2 = snd_soc_read(codec,
			MSM8X16_WCD_A_ANALOG_MBHC_ZDET_ELECT_RESULT);
	while (result2 & 0x01)  {
     #ifndef CONFIG_HEADSET_EUO_US_SWITCH
...
static void wcd_mbhc_detect_plug_type(struct wcd_mbhc *mbhc)
{
	...
	WCD_MBHC_RSC_ASSERT_LOCKED(mbhc);
...
#ifdef CONFIG_HEADSET_EUO_US_SWITCH
       pr_debug("%s: start ts3a225e detect\n", __func__);
       ...
       pr_debug("%s: ts3a225e reg 0x02 = 0x%x\n", __func__, ts3a225e_reg[1]);
       pr_debug("%s: ts3a225e reg 0x03 = 0x%x\n", __func__, ts3a225e_reg[2]);
       pr_debug("%s: ts3a225e reg 0x05 = 0x%x\n", __func__, ts3a225e_reg[4]);
       pr_debug("%s: ts3a225e reg 0x06 = 0x%x\n", __func__, ts3a225e_reg[5]);
       ...
       if (ts3a225e_reg[5] == 0x01) {
            ...
            if ((ts3a225e_reg[4]&0x07) == 0x07 || (ts3a225e_reg[4]&0x38) == 0x38) {
                ...
                pr_info("%s: ts3a225e is special headset", __func__);
            }
            goto exit;
       } else {
            pr_err("%s: ts3a225e detect fail\n", __func__);
       }
#endif
