...
#ifdef DATA_PARTITION_PROTECT_ENABLE
...
static int has_free_blocks(s64 free_blocks, s64 dirty_blocks)
{
	...
	if((free_blocks - dirty_blocks) < 5120) {
				  ...
				  s64 nclusters, unsigned int flags)
{
	...
	#ifdef DATA_PARTITION_PROTECT_ENABLE
	//pr_err("ext4_has_free_blocks: %lld, %lld, %lld, %u\n", free_clusters, dirty_clusters, rsv, sbi->s_groups_count);
	dirty_clusters += rsv;
